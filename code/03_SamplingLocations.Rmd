...
```{r}
#SS <- getNOAA.bathy(-123.6, -122, 46.4, 48.5, resolution = .3, antimeridian = FALSE)
#SS_xyz <- as.xyz(SS)
##SS_xyz$V3[SS_xyz$V3 > 1] <- (SS_xyz$V3[SS_xyz$V3 > 1])/10
#SS_xyz$V3[SS_xyz$V3 > -1] <- -1*(min(SS_xyz$V3))


#but get bathy for the whole west coast so that the depth color scales match for inset and main plot
SS <- getNOAA.bathy(-130, -119, 33, 53, resolution = 0.1, antimeridian = FALSE)
SS_xyz <- as.xyz(SS)
SS_xyz$V3[SS_xyz$V3 > -1] <- -1*(min(SS_xyz$V3))



seattle_location <- tibble(landmark = c("Seattle, WA", "University of Washington"),
...
p <- ggplot(data=metadataCOUNTS, aes(x = Longitude, y = Latitude)) + 
  geom_tile(data = SS_xyz, aes(x = V1, y = V2, fill = V3), inherit.aes = F) +
    scale_fill_gradient2(low="#0D0D47", mid="gainsboro", high="palegreen4",guide="none") +
  ...
  geom_point(data=seattle_location[seattle_location$landmark == "Seattle, WA",], aes(x = longitude, y = latitude), shape = 1, size = 5, color = "black") + 
  geom_label_repel(data=seattle_location[seattle_location$landmark == "Seattle, WA",], aes(label = landmark, x = longitude, y = latitude), point.padding = 5, alpha = 1, min.segment.length = 0, size = 3, label.padding = 0.2, box.padding = 0.2) +
  guides(
...
##add west coast of NA
WC <- getNOAA.bathy(-130, -119, 33, 53, resolution = 3, antimeridian = FALSE)
WC_xyz <- as.xyz(WC)
WC_xyz$V3[WC_xyz$V3 > -1] <- -1*(min(WC_xyz$V3))

p_wc <- ggplot(WC_xyz, aes(x = V1, y = V2, fill = V3)) + 
  geom_tile(data = WC_xyz, aes(x = V1, y = V2, fill = V3), inherit.aes = F) +
    scale_fill_gradient2(low="#0D0D47", mid="gainsboro", high="palegreen4",guide="none") +
# scale_fill_stepsn(colors = c("#1874CD", "#E8E8E8", "#458B00", "#006400"), values = c(min(SS_xyz$V3), 0, 10, max(SS_xyz$V3))) +
  #scale_fill_gradientn(colors = c("#1874CD", "#E8E8E8", "palegreen4"), 
                       #breaks = c(min(SS_xyz$V3), 0, 10, max(SS_xyz$V3)), 
                       #values = c(min(SS_xyz$V3), 0, 10, max(SS_xyz$V3))
   #                    ) +
   geom_contour(data = WC_xyz, 
               aes(x = V1, y = V2, z = V3),
               breaks = c(0), color = "grey5", linewidth = 0.3, inherit.aes = F) +
  geom_contour(data = WC_xyz, 
               aes(x = V1, y = V2, z = V3),
               binwidth = 1000, color = "grey65", linewidth = 0.1, inherit.aes = F) +
  coord_sf(xlim = c(-129, -122), 
           ylim = c(39, 53)) +
  ...
  scale_y_continuous(breaks = seq(40, 52, by = 4)) +
  scale_x_continuous(breaks = seq(-129, -122, by = 3)) +
  annotate("rect", xmin = -123.7, xmax = -122.1, ymin = 47.1, ymax = 48.4, alpha = 0.1, color = "black", linewidth = 0.5)

#p_wc

p_inset <- p + annotation_custom(ggplotGrob(p_wc), xmin = -123.7, xmax = -123.25,
                       ymin = 47.15, ymax = 47.9)
#p_inset

#no good b/c the axes don't align well
#p_both <- ggpubr::ggarrange(p_wc, p, nrow = 1, widths = c(1, 1.8), heights = c(1,1), labels = "auto", align = "h")
#p_both


jpeg("~/Documents/Rockfish/Conservation_Letters/Figures/Fig3map_rev.jpeg", width = 6, height = 6, units = "in", res = 300)
p_inset
dev.off()

pdf(file = "~/Documents/Rockfish/Conservation_Letters/Figures/Fig3map_rev.pdf", height = 6, width = 6)
p_inset
dev.off()
